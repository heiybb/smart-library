...
class DatabaseUtils:
    ...
    def insert_user(self, username, original_pass, first_name, last_name, email):
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO USER (username, hash_password,first_name,last_name,email) VALUES (%s, %s, %s, %s, %s)",
    ...
    def check_is_exist(self, pre_ck_username):
        ...
        """
        with self.connection.cursor() as cursor:
            cursor.execute("SELECT * FROM USER WHERE username = %s", (pre_ck_username,))
        self.connection.commit()
        #
        return cursor.rowcount == 1
    ...
    def get_user_detail(self, username):
        ...
        """
        with self.connection.cursor() as cursor:
            cursor.execute("SELECT username,first_name,last_name FROM USER WHERE username = %s", (username,))
            row = cursor.fetchone()
